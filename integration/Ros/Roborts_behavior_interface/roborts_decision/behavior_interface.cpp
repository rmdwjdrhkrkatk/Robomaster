#include <ros/ros.h>

#include "executor/chassis_executor.h"
#include "icra_roboin_msgs/SetBehavior.h"
#include "example_behavior/back_boot_area_behavior.h"
#include "example_behavior/escape_behavior.h"
#include "example_behavior/chase_behavior.h"
#include "example_behavior/search_behavior.h"
#include "example_behavior/patrol_behavior.h"
#include "example_behavior/goal_behavior.h"


int command = 0;
bool serviceCB(icra_roboin_msgs::SetBehavior::Request &req, icra_roboin_msgs::SetBehavior::Response &res) {
  int command_ = req.behavior;
  if (command != command_){
    command = command_;
    res.received = false;
  } else {
    res.received = true;
  }
  
  
  return true;
}

int main(int argc, char **argv) {
  ros::init(argc, argv, "behavior_interface");
  ros::NodeHandle nh;
  ros::ServiceServer service = nh.advertiseService("behavior_select_service",serviceCB);
  
  std::string full_path = ros::package::getPath("roborts_decision") + "/config/decision.prototxt";

  auto chassis_executor = new roborts_decision::ChassisExecutor;
  auto blackboard = new roborts_decision::Blackboard(full_path);

  roborts_decision::BackBootAreaBehavior back_boot_area_behavior(chassis_executor, blackboard, full_path);
  roborts_decision::ChaseBehavior        chase_behavior(chassis_executor, blackboard, full_path);
  roborts_decision::SearchBehavior       search_behavior(chassis_executor, blackboard, full_path);
  roborts_decision::EscapeBehavior       escape_behavior(chassis_executor, blackboard, full_path);
  roborts_decision::PatrolBehavior       patrol_behavior(chassis_executor, blackboard, full_path);
  roborts_decision::GoalBehavior       goal_behavior(chassis_executor, blackboard);

  
  ros::Rate rate(10);
  while(ros::ok()){
    ros::spinOnce();
    ROS_INFO("%d",command);
    switch (command) {
      //back to boot area
      case 1:
        back_boot_area_behavior.Run();
        break;
        //patrol
      case 2:
        patrol_behavior.Run();
        break;
        //chase.
      case 3:
        chase_behavior.Run();
        break;
        //search
      case 4:
        search_behavior.Run();
        break;
        //escape.
      case 5:
        escape_behavior.Run();
        break;
        //goal.
      case 6:
        goal_behavior.Run();
        break;
        //cancel all
      case 7:
        back_boot_area_behavior.Cancel();
        patrol_behavior.Cancel();
        chase_behavior.Cancel();
        search_behavior.Cancel();
        escape_behavior.Cancel();
        goal_behavior.Cancel();
        break;
      case 27:
        ROS_INFO("termination command");
        return 0;
      default:
        break;
    }
    rate.sleep();
  }


  return 0;
}


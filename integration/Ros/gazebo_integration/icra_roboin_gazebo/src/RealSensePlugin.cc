...
void RealSensePlugin::Load(physics::ModelPtr _model, sdf::ElementPtr /*_sdf*/)
{
  ...
  this->rosPub_ = this->rosNh_->advertise<sensor_msgs::Image>(this->dataPtr->rsModel->GetName() +  "realsense_rgb_image",4);
  this->rosPub_depth = this->rosNh_->advertise<sensor_msgs::Image>(this->dataPtr->rsModel->GetName() + "realsense_depth_image",4);	
}

...
class A2CAgent:
    ...
    def build_actor(self):
        actor = Sequential()
        actor.add(Conv1D(filters=10, kernel_size=self.state_size, activation='tanh',
    ...
    def build_critic(self):
        critic = Sequential()
        critic.add(Conv1D(filters=10, kernel_size=self.state_size, activation='relu',
                          input_shape=(self.state_size, 3), kernel_initializer='he_uniform'))
...
if __name__ == "__main__":
    ...
    for e in range(1, EPISODES + 1):
        ...
        while not done:
            ...
            #     print('policy : ', policy, 'action : ', action, 'score : ', score, 'advantage : ', advantage, 'reward', reward, 'value : ', value)
            ...
            if done:
                ...
                print("episode:", e, "score: ", round(score,2), "step : ", step, "global_step: ", global_step, "Goal : ", goal, "Dead : ", remain_human, "Remain oil : ", remain_oil)
                print('Total Dead : ', total_dead, 'Total Remain Oil : ', total_remain_oil)
        # 20 에피소드마다 모델 저장
        # if e % 20 == 0:
        #     agent.actor.save_weights("./save_model/grid_actor.h5")
        #     agent.critic.save_weights("./save_model/grid_critic.h5")
    print('Mean Step : ', (global_step/EPISODES), 'Survival rate : ', (1-total_dead/(3*EPISODES)), 'Collection rate : ', (1-total_remain_oil/(3*EPISODES)))
...
void RealSensePlugin::Load(physics::ModelPtr _model, sdf::ElementPtr /*_sdf*/)
{
  ...
  this->dataPtr->newColorFrameConn =
      this->dataPtr->colorCam->ConnectNewImageFrame(
          std::bind(&RealSensePlugin::OnNewColorFrame, this, this->dataPtr->colorCam,
                    this->dataPtr->colorPub));
...
void RealSensePlugin::OnNewDepthFrame() const
{
  ...
  //////rosimg.height = this->dataPtr->depthCam->ImageHeight();
  //////rosimg.width = this->dataPtr->depthCam->ImageWidth();
  msg.mutable_image()->set_pixel_format(common::Image::L_INT16);
  //////rosimg.encoding=cam->"rgb16";
  //////rosimg.is_bigendian =false;
  ...
  //////rosimg.step = (this->dataPtr->depthCam->ImageWidth() * this->dataPtr->depthCam->ImageDepth());
  //////rosimg.data = std::vector<unsigned char>(this->dataPtr->depthMap.data(),sizeof(*this->dataPtr->depthMap.data()) * imageSize);

  // Publish realsense scaled depth map
  this->dataPtr->depthPub->Publish(msg);
  //////rosPub_depth.publish(rosimg);
}

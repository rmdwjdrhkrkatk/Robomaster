...
static bool Set_MultiTracker(Ptr<MultiTracker>& multiTracker, string trackerTypes , VisionDataset& dataset , Ptr<Tracker>& tracker) {
	...
	std::cout << "(Set_MultiTracker)Total_Number_of_tracking:"<< Total_Trakcing_num << std::endl;

	if (flage != Total_Trakcing_num) {
		std::cout << "(Set_MultiTracker)KCF_Trakcer`s box number has porblem" << std::endl;
...
static bool KCFTraker_Run( VisionDataset& dataset , Ptr<MultiTracker> multiTracker )  //error could be occur 
{
...
#ifdef DEBUG
	std::cout << "(KCFTraker_Run)_start "<< std::endl;
#endif
	...
	if (objects != (dataset.number_of_detection + dataset.armour[1].number_of_armer + dataset.armour[2].number_of_armer)) {
		dataset.number_of_detection = 0;
#ifdef DEBUG
		std::cout << "(KCFTraker_Run) The output number of object != number of input  " << std::endl;
#endif
		return false;
	}
	...
	for (index = 0; index < objects; index++) {
		...
		if (flage != 100) {  //robot_state
			...
			*/
			...
			// this is number 1`s armour data
...
#ifdef DEBUG
			std::cout << "(KCFTraker_Run)Armour 1 is found:" << dataset.armour[1].number_of_armer << std::endl;
#endif
			...
			// this is number 2`s armour data
			dataset.armour[2].Center_x[index - dataset.number_of_detection - dataset.armour[1].number_of_armer] = (result[index].x + 0.5 * result[index].width);
			dataset.armour[2].Center_y[index - dataset.number_of_detection - dataset.armour[1].number_of_armer] = (result[index].y + 0.5 * result[index].height);
			dataset.armour[2].height[index - dataset.number_of_detection - dataset.armour[1].number_of_armer] = result[index].height;
			dataset.armour[2].width[index - dataset.number_of_detection - dataset.armour[1].number_of_armer] = result[index].width;
#ifdef DEBUG
			std::cout << "(KCFTraker_Run)Armour 2 is found:" << dataset.armour[2].number_of_armer << std::endl;
#endif
...
#ifdef DEBUG
	std::cout << "(KCFTraker_Run)Tracking complete, detected:" << dataset.number_of_detection << std::endl;
#endif
#ifdef DISPLAY

...
static bool configure_number_robot(VisionDataset data) {
	...
	for (index = 1; index < 3; index++) {
		...
		confidence_of_2 = 0;
		for (index2 = 0; index2 < 6; index2++) {
			if (data.armour[index].armour_type[index2] == Number1) {
				number_of_1++;
				confidence_of_1 = confidence_of_1 + data.armour[index].armour_confidence[index2];
			}
			if (data.armour[index].armour_type[index2] == Number2) {
				number_of_2++;
				confidence_of_2 = confidence_of_2 + data.armour[index].armour_confidence[index2];
			}
		}
#ifdef DEBUG

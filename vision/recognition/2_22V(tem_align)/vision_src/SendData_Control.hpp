...
///////////////////////////////making Vision angle//////////////
	if (X < (Width)) {
		hehe = (Width - X)*tan(a) / Width;
		x = atan(hehe);
		send.angle_hori = (-1 * (x * 100)) * 180 / 3.14159;  //minus is left
		...
		hehe = (X - Width)*tan(a) / Width;
		x = atan(hehe);
		send.angle_hori = 1 * (x * 100) * 180 / 3.14159;  //pluse is right 
	}

	if (Y < (Height)) {
		hehe = (Height - Y)*tan(a * 0.75) / Height;
		x = atan(hehe);
		send.angle_verti = 1 * (x * 100) * 180 / 3.14159;
		...
		hehe = (Y - Height)*tan(a * 0.75) / Height;
		x = atan(hehe);
		send.angle_verti = -1 * (x * 100) * 180 / 3.14159;
	}
///////////////////////////////////////////////////
...
// this function must be use after opperation for pixeltodata(), and yamiinfo data must have distance value
	...
	float angle_verti = send.angle_verti * 3.14159 / 180 ;
	float angle_hori = send.angle_hori * 3.14159 / 180;
	...
	if (send.Pixel_X < (width)) {
		send.real_width = (send.distance) * sin(angle_verti) * (send.Pixel_width) / (width - (send.Pixel_X));
	}
	else if (send.Pixel_X > (width)) {
		send.real_width = (send.distance) * sin(angle_verti) * (send.Pixel_width) / ((send.Pixel_X) - width);
	}
	else {
	...
	if (send.Pixel_Y < (height)) {
		send.real_height = (send.distance) * sin(angle_hori) * (send.Pixel_height) / (height - send.Pixel_Y);
	}
	else if (send.Pixel_Y > (height)) {
		send.real_height = (send.distance) * sin(angle_hori) * (send.Pixel_height) / (send.Pixel_Y - height);
	}
	else {
		//we have to think another algo to do this (i think about using ratio)
	}
...
#ifdef DEBUG
	std::cout << "(armourdata_selector)_is worked"<<"// selected target:" << target << " X_center data:"<< "data.armour[target].Center_x[count]"<<std::endl;
#endif

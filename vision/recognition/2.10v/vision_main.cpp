...
#include "roborts_msgs/PitchGain.h"
#include "roborts_msgs/YawGain.h"
...
int prev_target_index = 0;


bool SetYawGainCallback(roborts_msgs::YawGain::Request &req,
        roborts_msgs::YawGain::Response &resp ) {

    if(req.p != 0) YAW_P_GAIN = req.p;
    if(req.i != 0) YAW_I_GAIN = req.i;
    if(req.d != 0) YAW_D_GAIN = req.d;

    resp.received = true;
    return true;
}

bool SetPitchGainCallback(roborts_msgs::PitchGain::Request &req,
        roborts_msgs::PitchGain::Response &resp) {

    if(req.p != 0) PITCH_P_GAIN = req.p;
    if(req.i != 0) PITCH_I_GAIN = req.i;
    if(req.d != 0) PITCH_D_GAIN = req.d;
...
///////////////////////////////  deeplearning setting ///////////////////////////////
    ...
    std::string  weights_file = "/home/nvidia/CLionProjects/vision_dev/yolo/yolov3-tiny_20000.weights";
...
///////////////////////////////  start camera ///////////////////////////////
		...
		rs2::video_frame color_frame = processed.first_or_default(align_to);
...
/////////////////////////////// detecting ///////////////////////////////
			...
			if ((dataset.Center_X[1] > 0) or (dataset.Center_X[2] > 0)) {
				std::cout<<"enemy data rearragne"<<std::endl;
				//if there are enemy data
				VisionDataset_KCFCORMaker(dataset);
				armour_assign(detect_Data, dataset);
...
///////////////////////////////  escape_Traking///////////////////////////////
		...
		if (dataset.number_of_detection == 0) {
			dataset.Detection = true;
			std::cout << "dataset.number_of_detection == 0 , detecting mode on" << endl;
		}
...
//////////////////////////////////////////////////////////////////////////////////////////////
	...
	// ros pixel control start //
		if(dataset.number_of_detection > 0) {  // may not be working

			// TODO when target changes -> reinit i_error
//			if(gimbal.assinged_number == prev_target_index) {
//
//			} else {
//				gimbal.assinged_number = prev_target_index;
//			}

			std::cout << "activating pixel control" << std::endl;

			short pixel_error_x = gimbal.Pixel_X - X_CENTER_PIXEL;
			short pixel_error_y = gimbal.Pixel_Y - Y_CENTER_PIXEL;

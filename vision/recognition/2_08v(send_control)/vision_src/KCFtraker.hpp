...
static bool Set_MultiTracker(Ptr<MultiTracker>& multiTracker, string trackerTypes , VisionDataset& dataset , Ptr<Tracker>& tracker) {
	...
	for (index = 1; index < 3; index++) {  //accese enemy robot data
		for (index2 = 0; index2 = data.armour[index].number_of_armer; index2++) { //I think it could be need to control zero value
			Rect2d bbox(data.armour[index].Center_x[index2] - 0.5 * data.armour[index].width[index2], data.armour[index].Center_y[index2] - 0.5 * data.armour[index].height[index2]
			, data.armour[index].width[index2] , data.armour[index].height[index2]);

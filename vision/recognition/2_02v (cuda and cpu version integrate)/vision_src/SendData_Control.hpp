...
/* control pixel data and real angle*/
static int pixeltodata(short X , short Y , short w, short h , yamiinfo& send) {
	//realsense data (Dapth) hori angle 65.2 verti angle 58 diagonal 94 error +-3
	//RGB viewing angle horiangle 69.4 veri angle 42.5 diagonal 77 erroe +-3
	// but we need only RGB angle 
	// 60degree * pi/180 = radian. 
...
static int realsize_control( yamiinfo& send) {
...
static void sendData_Maker(VisionDataset& data , sendDataset& send) {
	...
	send.data_0.infochecker = pixeltodata(data.Center_X[0], data.Center_Y[0],data.width[0],data.height[0] ,send.data_1);
	send.data_1.infochecker = pixeltodata(data.Center_X[1], data.Center_Y[1], data.width[1], data.height[1], send.data_2);
	send.data_2.infochecker = pixeltodata(data.Center_X[2], data.Center_Y[2], data.width[2], data.height[2], send.data_3);
	send.data_0.distance = data.distance[0] ;
	send.data_1.distance = data.distance[1] ;
	send.data_2.distance = data.distance[2] ;
	realsize_control(send.data_0);
